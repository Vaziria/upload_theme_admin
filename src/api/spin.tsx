...
export async function getSpinConfig (): Promise<{
    ...
    const res = await client.get('/legacy/api/settingSpin')
...
export async function exampleSpin (title: string): Promise<string> {
    const params = { title }
    const res = await client.get('/legacy/v1/examplespin/title', { params })
    return res.data.text
}

export async function settingSpin (titlePool: Spin[]): Promise<void> {
    await client.post('/legacy/api/config/settingSpin', {
        name: 'settingSpin',
        titlePool
    })
}
...
export async function configSpin (payload: SpinConfig): Promise<void> {
    await client.post('/legacy/api/settingSpin', payload)
}

export async function deleteSpin (name: string): Promise<void> {
    const params = { name }
    await client.delete('/legacy/api/dataspin', { params })
}

export async function getDataspin (): Promise<string[]> {
    const res = await client.get('/legacy/api/dataspin')
    return res.data
}

export async function getDataspinData (name: string): Promise<{ data: string[], name: string }> {
    const params = { name }
    const res = await client.get('/legacy/api/dataspin', { params })
...
export async function postDataspin (name: string, data: string[]): Promise<void> {
    await client.post('/legacy/api/dataspin', {
        name,
        data
    })
}
