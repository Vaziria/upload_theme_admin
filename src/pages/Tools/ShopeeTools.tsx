...
const ShopeeTools: React.FC<Props> = (props: Props) => {
    ...
    function applySaveTempAkun(onSuccess?: (res: BaseWebResponse) => void) {
        saveTempAkun({
            onError: errHandler,
            onSuccess(res) {
                props.onSuccess("akunlist saved...")
                onSuccess?.(res)
            }
        }, { data: akuns })
    }
    ...
    function applyCheckorder(query: CheckOrderQueryCli) {
        applySaveTempAkun(() => {
            checkorder({
                query,
                onError: errHandler,
                onSuccess: () => props.onSuccess("check order running..."),
            })
        })
    }
        ...
        applySaveTempAkun(() => saveDeleteConfig({
            ...
            onSuccess: () => saveDeleteConfigProduct({
                ...
                onSuccess: () => deleteProduct({
                    query,
                    onError: errHandler,
                    onSuccess: () => props.onSuccess("delete produk running...")
                })
            }, configProd)
        }, config))
    }
    ...
    function applyTokoLibur(query: TokoLiburQueryCli) {
        applySaveTempAkun(() => tokoLibur({
            query,
            onError: errHandler,
            onSuccess: () => props.onSuccess("toko libur running...")
        }))
    }
    ...
    return <Card title="Shopee Tools">
